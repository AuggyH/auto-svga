...
test("short-term host actions block unicode-normalized Save As target aliases", async () => {
  const sourcePath = "/Users/designer/private/caf\u00e9.svga";
  const normalizedTargetPath = "/Users/designer/private/cafe\u0301.svga";
  const sourceBytes = await createShortTermOptimizableSvgaFixture();
  const host = createMemoryHost({
    [sourcePath]: sourceBytes
  });
  ...
  assert.ok(blockedSaveAs.activeOutputBytes);
  assert.equal(sha256(host.snapshot(sourcePath)), sha256(sourceBytes));
  assert.throws(() => host.snapshot(normalizedTargetPath), /missing snapshot/);
});
