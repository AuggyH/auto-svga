...
- Added S8/S9/S10 optimization result proof: smoke now opens the optimizer fixture, runs the real safe-optimization workflow, requires an optimized output, visible before/after comparison, metrics, enabled Save As, and unchanged source bytes.
- Expanded the optimization result card so designers can see concrete executed actions and skipped/review-only methods, not just a generated optimized copy and size metrics.
...
- Added S11/S12 replaceable workflow proof. Smoke now uses a designer-named runtime fixture to prove context-menu imageKey rename with Enter confirmation, dirty rename output, replacement preview output, enabled Save As, Reset availability, Reset restoration, and source immutability.
...
- Short-term optimization proof: pass; `.artifacts/product/short-term/short-term-optimization-proof.json` records two safe actions, 302 B -> 242 B output size, unchanged source SHA, changed optimized SHA, visible metrics, visible executed-action rows, visible skipped-method rows, visible before/after comparison, and enabled Save As.
- Short-term rename proof: pass; `.artifacts/product/short-term/short-term-rename-proof.json` records `profile_frame -> profile_frame_renamed`, context-menu entry, Enter confirmation, enabled Save As, changed output SHA, and unchanged source SHA.
- Short-term replacement proof: pass; `.artifacts/product/short-term/short-term-replacement-proof.json` records replacement PNG hash, changed output SHA, enabled Save As before Reset, Reset command availability, reset-to-original SHA, cleared dirty output, and unchanged source SHA.
