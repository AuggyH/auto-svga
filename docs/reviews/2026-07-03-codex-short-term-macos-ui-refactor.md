...
- Rebound the normal App proof driver from legacy Workbench selectors to the short-term macOS client: it now opens the fixture through the short-term host action, waits for the Preview canvas, Overview facts, and asset list, and captures the normal loaded screenshot from the new UI.
...
- Normal App proof: pass; `AUTO_SVGA_DESKTOP_NORMAL_PROOF` reports `hostOpen=true`, `playback=true`, `canvasNonBlank=true`, `inspectionReport=true`, `auditPanel=true`, `localOnly=true`, and `noCspViolation=true` against the short-term macOS client.
