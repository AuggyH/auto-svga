...
| 4 | Keep the fix scoped to packaging/runtime dependency closure, not UI or product behavior. | Done |
| 5 | Preserve the short-term client optimization, rename, replacement, and save flow behavior. | Verified by targeted tests and packaged smoke |
...
## 7. Risks
- This fixes the known `protobufjs` package resolution failure. If future runtime modules import additional external packages, those packages must be added to the runtime dependency closure and package assertion in the same change.
