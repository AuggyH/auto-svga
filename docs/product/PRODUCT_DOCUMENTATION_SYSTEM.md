...
| How should parallel agents coordinate foreground desktop clients and avoid owner-baseline drift? | `docs/engineering/DESKTOP_CLIENT_COORDINATION_PROTOCOL.md` | Foreground launch, concurrent client instance, foreground lease, owner local stable baseline, or local stable replacement rules change; must not redefine product scope |
...
| `docs/engineering/DESKTOP_CLIENT_COORDINATION_PROTOCOL.md` | Active desktop-client coordination protocol | Defines foreground leases, client-instance identity, owner local stable baseline handling, and baseline-drift gates; cannot redefine product scope from the main PRD. |
