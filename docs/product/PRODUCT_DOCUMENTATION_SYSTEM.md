...
Status: active documentation governance for the short-term macOS client mainline
...
1. Latest explicit Product Owner correction.
...
3. GitHub Issues, pull requests, and Actions for current task, candidate, and
   validation state. These volatile records may not redefine roadmap scope.
...
6. Historical docs, autonomous-run snapshots, frozen baselines, old review
   packets, and older status
...
| How should parallel agents coordinate foreground desktop apps, shared macOS UI resources, and owner-baseline drift? | `docs/engineering/DESKTOP_CLIENT_COORDINATION_PROTOCOL.md` | Foreground launch, concurrent app/client instance, Finder/Open dialog, After Effects UI, clipboard, foreground lease, owner local stable baseline, or local stable replacement rules change; must not redefine product scope |
...
| How should source-level implementation quality, architecture risk, dependency risk, and high-risk code handoffs be reviewed? | `docs/engineering/CODE_REVIEW_WORKFLOW.md` | Code review states, required review inputs, high-risk source areas, reviewer authority, QA interaction, or first code-health audit rules change; must not redefine product scope |
...
| What is true about current implementation work? | GitHub Issues, pull requests, and Actions | Candidate, blocker, owner, validation, or merge state changes; must not redefine the roadmap |
...
| `docs/engineering/DESKTOP_CLIENT_COORDINATION_PROTOCOL.md` | Active desktop foreground and client coordination protocol | Defines global foreground input leases, shared macOS UI resource handling, client/external-app identity, owner local stable baseline handling, and baseline-drift gates; cannot redefine product scope from the main PRD. |
