...
| R6 | `1977cbc` | Final allowed repair: stricter inventory/state/motion/normal App/handoff | Medium: shared state behavior; invalid alias repair | Medium: visible normal app startup path | High: strict evidence gates, mutation tests, final parity | High: registry final binding and handoffs | High: final package, privacy and stale-root repairs | HUMAN_REQUIRED; evidenceCompleteness COMPLETE; all machine reports PASS | TECHNICAL_REVIEW_REQUIRED; product not accepted | Loading/Empty still unclear; Invalid stale state; multi-source flow unproven; comparedPixels=0 PASS risk; missing true interaction trace; identical motion frames; Reviewer B not independent; normal App proof incomplete | Evidence system, state, multi-source, motion, reviewer, app, handoff, and end-to-end ownership classes repeated | All core findings remain open | Best evidence state so far; portable package improved | R6 external review rejected owner acceptance after budget exhausted; Repair 7 is not allowed |
