...
const securityHeaders = {
  ...
  "content-security-policy": "default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self'; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
...
export async function startPrototypeServer({ appRoot, reportToken }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && requestUrl.pathname === "/health") {
      return sendJson(response, 200, { status: "ok", runtime: "electron-prototype" });
    }
  ...
  if (!address || typeof address === "string") throw new Error("Prototype server did not bind a TCP port");
