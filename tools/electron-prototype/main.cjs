...
const sessionRoot = path.join(os.tmpdir(), `auto-svga-electron-prototype-${process.pid}`);
...
function validateSmokeResult(value) {
  ...
  const keys = ["localPage", "playback", "canvasNonBlank", "inspectionReport", "auditPanel", "localOnly"];
...
function redactLogMessage(value) {
  return String(value)
    .replaceAll(sessionRoot, "<prototype-session>")
...
async function finishSmoke(window, result) {
  ...
  console.log(`AUTO_SVGA_ELECTRON_SMOKE ${JSON.stringify({ ...result, passed })}`);
  await prototypeServer.close();
  window.destroy();
  app.exit(passed ? 0 : 1);
}

async function createPrototypeWindow() {
  const { startPrototypeServer } = await import(
    pathToFileURL(path.join(appRoot, "server.mjs")).href
  );
  prototypeServer = await startPrototypeServer({ appRoot, reportToken });
  expectedOrigin = prototypeServer.origin;
  ...
  window.webContents.on("console-message", (event, ...legacyArguments) => {
    ...
    console.log(`AUTO_SVGA_RENDERER ${redactLogMessage(message)}`);
  });
  window.webContents.on("will-navigate", (event, url) => {
    if (!url.startsWith(`${expectedOrigin}/`)) event.preventDefault();
  });

  ipcMain.handle("prototype:smoke-result", async (event, input) => {
...
app.whenReady().then(createPrototypeWindow).catch((error) => {
  console.error(`AUTO_SVGA_ELECTRON_ERROR ${redactLogMessage(error instanceof Error ? error.message : error)}`);
  app.exit(1);
});

app.on("window-all-closed", async () => {
  if (prototypeServer) await prototypeServer.close();
  app.quit();
});
