...
test("default Electron renderer is the short-term macOS client and keeps legacy Workbench isolated", async () => {
  ...
  assert.match(shortTermDomRenderers, /nodes\.fileIdentity\.textContent = displayName/);
  assert.match(shortTermDomRenderers, /nodes\.playbackMeta\.textContent = playbackMeta/);
  assert.match(shortTermDomRenderers, /nodes\.discardMessage\.textContent = copy/);
  assert.match(shortTermDomRenderers, /nodes\.errorMessage\.textContent = copy/);
