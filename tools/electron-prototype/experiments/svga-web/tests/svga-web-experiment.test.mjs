...
test("default Electron renderer is the short-term macOS client and keeps legacy Workbench isolated", async () => {
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-edit-reserved-renderers\.mjs"/);
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-overview-renderers\.mjs"/);
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-compare-surface\.mjs"/);
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-recent-files-surface\.mjs"/);
  assert.match(shortTermEntry, /refreshShortTermRecentFiles\(\{ bridge, nodes \}\)/);
  assert.match(shortTermEntry, /clearShortTermRecentFiles\(\{ bridge, nodes \}\)/);
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-resource-menu-surface\.mjs"/);
  ...
  assert.match(shortTermEntry, /from "\.\/short-term-macos-optimization-surface\.mjs"/);
  ...
  assert.match(shortTermEntry, /renderCompareInfo\("A", "A 文件", state\.model, state\.displayName\)/);
  assert.match(shortTermEntry, /renderShortTermGeneralComparePlaceholder\(nodes\)/);
  ...
  assert.match(shortTermSmokeRunner, /shortTermTabKeyboardProof/);
  ...
  assert.match(shortTermEventBindings, /runtimeTextInput\.addEventListener\("keydown"/);
  assert.match(shortTermEventBindings, /nodes\.textDialog\.close\("confirm"\)/);
  assert.match(shortTermEventBindings, /nodes\.textDialog\.close\("cancel"\)/);
  assert.match(shortTermSmokeProofModel, /initialFocusInput/);
  assert.match(shortTermSmokeProofModel, /focusReturnedAfterClose/);
  assert.match(shortTermSmokeRunner, /const runtimeTextInitialFocusInput = document\.activeElement === nodes\.runtimeTextInput/);
  assert.match(shortTermSmokeRunner, /const runtimeTextFocusReturnedAfterClose = document\.activeElement === nodes\.editTextButton/);
  ...
  assert.match(shortTermSmokeRunner, /collectShortTermTabCaptureState/);
  assert.match(shortTermSmokeRunner, /state\.smokeTabCaptureStates = \[\]/);
  assert.match(shortTermSmokeRunner, /const setSmokeTab = async \(tab, artifactName = ""\)/);
  assert.match(shortTermSmokeRunner, /setTab\(tab, \{ focus: true \}\)/);
  assert.match(shortTermSmokeRunner, /document\.activeElement\?\.blur\?\.\(\)/);
  assert.ok(shortTermSmokeRunner.includes('document.querySelector(`[data-tab="${tab}"]`)?.classList.contains("isSelected")'));
  assert.ok(shortTermSmokeRunner.includes('document.querySelector(`[data-panel="${tab}"]`)?.hidden === false'));
  ...
  assert.match(shortTermSmokeRunner, /await setSmokeTab\("optimization", "short-term-preview-optimization"\);[\s\S]*captureSmokeArtifact\("short-term-preview-optimization"\)/);
  assert.match(shortTermSmokeRunner, /await setSmokeTab\("replaceable", "short-term-preview-replaceable"\);[\s\S]*captureSmokeArtifact\("short-term-preview-replaceable"\)/);
  ...
  assert.match(shortTermSmokeRunner, /runtimeTextModalPlaybackBeforeSpace/);
