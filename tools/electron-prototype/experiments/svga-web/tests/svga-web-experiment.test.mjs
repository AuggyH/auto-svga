...
test("default Electron renderer is the short-term macOS client and keeps legacy Workbench isolated", async () => {
  ...
  assert.match(shortTermDomRenderers, /nodes\.optimizationSummary\.textContent = view\.summaryCopy/);
  assert.match(shortTermDomRenderers, /nodes\.runOptimizationButton\.textContent = view\.runButtonCopy/);
  assert.match(shortTermDomRenderers, /nodes\.runOptimizationButton\.title = view\.runButtonTitle/);
  assert.match(shortTermDomRenderers, /nodes\.runOptimizationButton\.disabled = view\.runButtonDisabled/);
  assert.match(shortTermDomRenderers, /nodes\.findingList\.replaceChildren/);
  assert.match(shortTermDomRenderers, /nodes\.findingList\.prepend/);
