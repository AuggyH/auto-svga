...
async function main() {
  ...
  const rightSurfaceContractChecks = [
    ...
    /--asv-component-fact-cell-label-size:\s*var\(--asv-type-size-caption\)/.test(tokens),
    /--asv-component-fact-cell-label-line-height:\s*16px/.test(tokens),
    /--asv-component-fact-cell-unit-size:\s*var\(--asv-type-size-caption\)/.test(tokens),
    /const requiredIds = new Set\(\[[\s\S]*"fileSize",[\s\S]*"decodedMemory",[\s\S]*"duration",[\s\S]*"canvas",[\s\S]*"fps"/.test(renderModel),
    ...
    /\.compareInfo\s*\{[\s\S]*gap: var\(--asv-right-panel-section-gap\)[\s\S]*min-width: 0[\s\S]*overflow-x: hidden[\s\S]*overflow-y: auto[\s\S]*padding: var\(--asv-compare-panel-padding-block-start\) var\(--asv-right-panel-padding\) var\(--asv-right-panel-padding\)[\s\S]*background: var\(--asv-side-surface-bg\)/.test(modules),
    /--asv-component-compare-panel-padding-block-start: var\(--asv-component-right-panel-padding\)/.test(tokens),
    /--asv-component-compare-title-size: var\(--asv-type-size-state-title\)/.test(tokens),
    /--asv-component-compare-file-name-color: var\(--asv-color-text-secondary\)/.test(tokens),
    /--asv-component-compare-file-name-weight: var\(--asv-type-weight-medium\)/.test(tokens),
    /\.compareSummary h2\s*\{[\s\S]*font-size: var\(--asv-compare-title-size\)[\s\S]*line-height: var\(--asv-compare-title-line-height\)/.test(modules),
    /\.comparePairHeader strong\s*\{[\s\S]*color: var\(--asv-compare-file-name-color\)[\s\S]*font-weight: var\(--asv-compare-file-name-weight\)[\s\S]*line-height: var\(--asv-compare-file-name-line-height\)/.test(modules),
  ...
  record("optimization-detail-uses-figma-r5-candidate-row-contract", /dataset\.component = "OptimizationFindingRow"/.test(optimizationRenderer)
    ...
    && /--asv-component-finding-row-badge-font-size:\s*var\(--asv-type-size-micro\)/.test(tokens)
    && /\.findingRow\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\) auto/.test(components)
    && !/findingImpact/.test(renderModel)
  ...
  record("figma-r9-compare-edit-settings-contract-covered", /--asv-component-preview-gap:\s*var\(--asv-base-space-0\)/.test(tokens)
    ...
    && /--asv-component-settings-action-radius:\s*var\(--asv-base-radius-12\)/.test(tokens)
    ...
    && /--asv-component-settings-choice-radius:\s*var\(--asv-base-radius-8\)/.test(tokens)
    && /--asv-component-settings-choice-selected-radius:\s*var\(--asv-base-radius-12\)/.test(tokens)
    ...
    && /--asv-component-settings-choice-selected-bg:\s*var\(--asv-color-status-info-bg\)/.test(tokens)
    && /--asv-component-settings-choice-selected-color:\s*var\(--asv-color-text-primary\)/.test(tokens)
    ...
    && /<span>当前文件暂无可用编辑操作<\/span>/.test(page)
    && /<span>可返回预览模式继续检查文件<\/span>/.test(page)
    && !/短期版本|后续规划/.test(page.match(/<section class="view editView"[\s\S]*?<\/section>\s*<div class="canvasToast"/)?.[0] ?? "")
  ...
  record("figma-r10-atom-molecule-contract-covered", /--asv-component-text-input-width:\s*172px/.test(tokens)
    ...
    && /--asv-component-file-header-action-width:\s*60px/.test(tokens)
    && /--asv-component-state-recovery-action-width:\s*72px/.test(tokens)
    && /--asv-component-playback-error-action-width:\s*72px/.test(tokens)
