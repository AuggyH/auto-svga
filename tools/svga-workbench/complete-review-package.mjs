...
async function writeReviewPacket(root, { headCommit, headTree, headShort, completeZipName, appZipName }) {
  await writeFile(path.join(root, "README.md"), [
    ...
    "",
    "Status: autonomous repair required and continue. This handoff is not Product Owner acceptance.",
    ""
  ].join("\n"), "utf8");
  await writeFile(path.join(root, "REVIEW_PACKET.md"), [
    ...
    "",
    "Review whether the current Phase 4 partial status is acceptable for the next autonomous implementation slice, and provide signing/notarization credentials only when trusted distribution is required.",
    ""
  ].join("\n"), "utf8");
  await writeFile(path.join(root, "FINAL_RESPONSE.txt"), [
    `SVGA Workbench v1 complete review directory regenerated at head ${headShort}.`,
    `Primary artifact: review/${completeZipName}`,
    `macOS App ZIP: app/${appZipName}`,
    "Status: repair package generated; Product Owner acceptance not claimed.",
    ""
  ].join("\n"), "utf8");
}
